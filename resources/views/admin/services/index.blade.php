...
<div class="rounded-xl bg-gray-900 border border-white/5 overflow-hidden">
    <table class="w-full text-sm">
        <thead><tr class="text-gray-400 border-b border-white/5"><th class="text-left p-4">Title</th><th class="text-left p-4">Price Range</th><th class="text-left p-4">Status</th><th class="text-right p-4">Actions</th></tr></thead>
        <tbody>
            @foreach($services as $service)
            <tr class="border-b border-white/5 hover:bg-white/5">
                <td class="p-4 font-medium">{{ $service->title }}</td>
                <td class="p-4 text-gray-400">{{ $service->price_range }}</td>
