...
@section('content')
<h1 class="text-2xl font-bold mb-6">{{ isset($category) ? 'Edit Category' : 'New Category' }}</h1>

<form action="{{ isset($category) ? route('admin.categories.update', $category) : route('admin.categories.store') }}" method="POST" class="max-w-2xl space-y-6">
    @csrf
    ...
    <div class="flex gap-3">
        ...
        <a href="{{ route('admin.categories.index') }}" class="bg-white/5 hover:bg-white/10 text-gray-300 font-medium px-6 py-3 rounded-xl transition-all">Cancel</a>
    </div>
</form>
@endsection
