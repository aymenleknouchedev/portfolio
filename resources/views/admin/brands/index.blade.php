@extends('layouts.admin')

@section('content')
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold">Brands</h1>
    <a href="{{ route('admin.brands.create') }}" class="bg-purple-600 hover:bg-purple-500 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-all">+ New Brand</a>
</div>
<div class="rounded-xl bg-gray-900 border border-white/5 overflow-hidden">
    <table class="w-full text-sm">
        <thead>
            <tr class="text-gray-400 border-b border-white/5">
                <th class="text-left p-4">Logo</th>
                <th class="text-left p-4">Name</th>
                <th class="text-left p-4">Order</th>
                <th class="text-left p-4">Status</th>
                <th class="text-right p-4">Actions</th>
            </tr>
        </thead>
        <tbody>
            @foreach($brands as $brand)
            <tr class="border-b border-white/5 hover:bg-white/5">
                <td class="p-4">
                    <img src="{{ asset('storage/' . $brand->logo) }}" alt="{{ $brand->name }}" class="h-8 w-auto object-contain bg-white/10 rounded px-2 py-1">
                </td>
                <td class="p-4 font-medium">{{ $brand->name }}</td>
                <td class="p-4 text-gray-400">{{ $brand->sort_order }}</td>
                <td class="p-4">
                    <span class="px-2 py-1 rounded-full text-xs {{ $brand->is_active ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400' }}">{{ $brand->is_active ? 'Active' : 'Inactive' }}</span>
                </td>
                <td class="p-4 text-right">
                    <a href="{{ route('admin.brands.edit', $brand) }}" class="text-purple-400 hover:text-purple-300 mr-3">Edit</a>
                    <form action="{{ route('admin.brands.destroy', $brand) }}" method="POST" class="inline" onsubmit="return confirm('Delete this brand?')">
                        @csrf @method('DELETE')
                        <button class="text-red-400 hover:text-red-300">Delete</button>
                    </form>
                </td>
            </tr>
            @endforeach
        </tbody>
    </table>
    @if($brands->isEmpty()) <p class="p-8 text-center text-gray-400">No brands yet.</p> @endif
</div>
@endsection
