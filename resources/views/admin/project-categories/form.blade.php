...
@section('page_title', isset($category) ? 'Edit Project Category' : 'New Project Category')
@section('page_subtitle', isset($category) ? $category->name : 'Create a new portfolio category')

@section('content')
<form action="{{ isset($category) ? route('admin.project-categories.update', $category) : route('admin.project-categories.store') }}" method="POST" class="space-y-6">
    @csrf
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 rounded-2xl bg-gray-900 border border-white/5 p-6 space-y-5">
            <header class="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">Group portfolio projects by type.</p>
                </div>
            </header>
        ...
        <aside class="space-y-4">
            <div class="card-accent p-6">
                ...
                <p class="text-xs text-gray-400 mb-4">Click below to save.</p>
                <button type="submit"
            ...
            <div class="rounded-2xl bg-gray-900 border border-white/5 p-6">
                ...
                <label class="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 cursor-pointer transition-colors">
                    ...
                    <div>
                        ...
                        <div class="text-[11px] text-gray-500">Visible on the portfolio page.</div>
                    </div>
                </label>
            </div>
        </aside>
